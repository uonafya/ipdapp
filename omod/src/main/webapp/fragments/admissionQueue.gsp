<script>
	...
	var getAdmissionQueue = function(){
		admissionQueueTableObject.find('td.dataTables_empty').html('<span><img class="search-spinner" src="'+ui.resourceLink('uicommons', 'images/spinner.gif')+'" /></span>');
		var requestData = {
			ipdWard:		'${ipdWard.id}',
	...
	var updateAdmissionQueueResults = function(results){
		...
		_.each(admissionQueueResultsData, function(result){
			...
			var icons = '<div style="position: static" class="dropdown"><span class="dropdown-name"><i class="icon-cog"></i>Actions<i class="icon-sort-down"></i></span><ul><li><a href="admissionForm.page?admissionId='+result.id+'&ipdWard='+result.admissionWard+'"><i class="icon-signin"></i>Admit</a></li><li><a class="remove-patient" data-idnt="'+result.id+'" data-action=1><i class="icon-remove"></i>Remove</a></li><li><a class="remove-patient" data-idnt="'+result.id+'" data-action=2><i class="icon-thumbs-down"></i>No bed</a></li></ul></div>';
